...
from datas import build_dataset, build_dataloader, build_iterator,extract,visualize,convert_to_color,display_predictions,count_sliding_window,sliding_window,grouper
import wandb
...
def worker(rank_gpu, args):
    ...
    best_epoch = 0
    best_PA = 0.
    # input = torch.randn(1, 48, 11, 11).cuda()
        ...
        if epoch > int(CFG.EPOCHS / args.world_size):
            ...
            if dist.get_rank() == 0:
                ...
                if CFG.DATASET.NAME!='ShangHang':
                    ...
                    if (num>1000):
                        num=1000
                    visualize(f_s.cpu().detach()[0:num], f_t.cpu().detach()[0:num],y=torch.cat((y_s[0:num],y_t[0:num]),0),label=list(range(1,NUM_CLASSES+1)),name="dsan")
                    visualize(f_s.cpu().detach()[0:num], f_t.cpu().detach()[0:num],y=torch.cat((label_s[0:num],label_t[0:num]),0),label=list(range(1,NUM_CLASSES+1)),name="dsan")
