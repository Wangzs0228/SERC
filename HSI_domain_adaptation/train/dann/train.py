...
from datas import build_dataset, build_dataloader, build_iterator, visualize,convert_to_color,display_predictions,count_sliding_window,sliding_window,grouper
import wandb
...
def extract(net, data_loader, device):
    ...
    if r_l.min()==0:
        r_l = r_l +1 
    return r_f, r_y ,r_l
    # return torch.cat(return_f), torch.cat(return_y)
        ...
        if epoch > int(CFG.EPOCHS / args.world_size):
            ...
            if dist.get_rank() == 0:
                ...
                if CFG.DATASET.NAME!='ShangHang':
                    f_s,y_s,label_s = extract(model, source_dataloader, "cuda",)
                    f_t,y_t,label_t = extract(model, val_dataloader, "cuda",)
                    # tSNE_filename = osp.join(logger.visualize_directory, 'TSNE.pdf')
                    num = np.minimum(len(f_s),len(f_t))
                    if (num>1000):
                        num=1000
                    visualize(f_s.cpu().detach()[0:num], f_t.cpu().detach()[0:num],y=torch.cat((y_s[0:num],y_t[0:num]),0),label=list(range(1,NUM_CLASSES+1)),name="dann")
                    visualize(f_s.cpu().detach()[0:num], f_t.cpu().detach()[0:num],y=torch.cat((label_s[0:num],label_t[0:num]),0),label=list(range(1,NUM_CLASSES+1)),name="dann")

                probabilities = test(model, target_dataset.data.permute(1,2,0), patch_size=CFG.DATASET.PATCH.HEIGHT ,batch_size=CFG.DATALOADER.BATCH_SIZE,n_classes=NUM_CLASSES,)
                ...
                input = torch.randn(1, source_dataset.data.shape[0], 11, 11).cuda()
                flops, params = profile(model, (input, 0))
