...
class PaviaDataset(HSIDataset):
    def __init__(self, root, split: str, window_size: Tuple[int, int], pad_mode: str, sample_num: int = None,
        ...
        if split == 'train':
            ...
            gt_filename = 'paviaU_gt_7.mat'
            
            self.data_path = os.path.join(root, data_filename)
            self.data = sio.loadmat(self.data_path)['paviaU'].astype('float32')
            self.gt_path = os.path.join(root, gt_filename)
            self.gt = sio.loadmat(self.gt_path)['paviaU_gt_7'].astype('int')
            self.gt_raw = self.gt.copy()
        else:
            # 验证集等于测试集
            data_filename = 'pavia.mat'
            gt_filename = 'pavia_gt_7.mat'
            self.data_path = os.path.join(root, data_filename)
            # N*W*C
            self.data = sio.loadmat(self.data_path)['pavia'].astype('float32')
            self.gt_path = os.path.join(root, gt_filename)
            self.gt = sio.loadmat(self.gt_path)['pavia_gt_7'].astype('int')
            self.gt_raw = self.gt.copy()
