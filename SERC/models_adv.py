...
from utils import grouper, sliding_window, count_sliding_window, camel_to_snake,GraphConvolution
...
def train(
    ...
    iter_data_adv_s=ForeverDataIterator(train_loader_adv_s)
    iter_data_adv_t=ForeverDataIterator(train_loader_adv_t)
    ...
    for e in tqdm(range(1, epoch + 1), desc="Training epoch"):
        ...
        for batch_idx in tqdm(range(0, total), desc="Training the network"):
            data, target,ord_s,weight_s=next(iter_data)
            data_t, target_t,ord_t,weight_t=next(iter_data_t)
            
            data_adv_s, _,_,_=next(iter_data_adv_s)
            data_adv_t, _,_,_=next(iter_data_adv_t)
            # Load the data into the GPU if required
            x_s, labels_s,ord_s,weight_s,data_adv_s = data.to(device), target.to(device),ord_s.to(device),weight_s.to(device),data_adv_s.to(device)
            x_t, labels_t,ord_t,weight_t,data_adv_t = data_t.to(device), target_t.to(device),ord_t.to(device),weight_t.to(device),data_adv_t.to(device)
            weight_s = (100 * weight_s / (torch.sum(weight_s)+0.001)).unsqueeze(dim=1)
            # labels_s  =  labels_s-1
            # labels_t  =  labels_t-1
            data = torch.cat((x_s, x_t), dim=0)
            data_adv = torch.cat((data_adv_s, data_adv_t), dim=0)
            ...
            if supervision == "full":
                ...
                # f_s,output_s = net(x_s,adj_s)
                # f_t,output_t = net(x_t,adj_t)
                # f   = torch.cat((f_s, f_t), dim=0)
                # output = torch.cat((output_s, output_t), dim=0)
                
                # grad_class= torch.autograd.grad(torch.max(output,1)[0].sum(),f,create_graph=True,retain_graph=True,)[0].detach()
                # grad_class_src,grad_class_tar= grad_class.chunk(2, dim=0)
                # flag_src_strong=np.ones_like(grad_class_src)*(grad_class_src>0)
                # flag_src_weak  =np.ones_like(grad_class_src)*(grad_class_src<0)
                # flag_src_strong=torch.tensor(flag_src_strong).cuda()
                # flag_src_weak  =torch.tensor(flag_src_weak).cuda()
                # flag_tar_strong=np.ones_like(grad_class_tar)*(grad_class_tar>0)
                # flag_tar_weak  =np.ones_like(grad_class_tar)*(grad_class_tar<0)
                # flag_tar_strong=torch.tensor(flag_tar_strong).cuda()
                # flag_tar_weak  =torch.tensor(flag_tar_weak).cuda()
                # flag_strong=torch.ones_like(grad_class).cuda()*(grad_class>0)
                # flag_weak  =torch.ones_like(grad_class).cuda()*(grad_class<0)  
                # f_strong,f_weak= flag_strong*f,flag_weak*f
                
                loss = (criterion(output_s, labels_s)*weight_s).mean()+hyperparams['mcc']*Align_dict['mcc'](output_t)
                # +hyperparams['mmd']*Align_dict['mmd'](f_s,f_t)\
                # +hyperparams['dann']*Align_dict['dann'](f.chunk(2, dim=0)[0], f.chunk(2, dim=0)[1])    
                # +hyperparams['cdan']*Align_dict['cdan'](output_s, f_strong.chunk(2, dim=0)[0], output_t, f_strong.chunk(2, dim=0)[1])\
                # +hyperparams['dann']*Align_dict['dann'](f_weak.chunk(2, dim=0)[0], f_weak.chunk(2, dim=0)[1])
            ...
            dann_losses[iter_]= Align_dict['dann'].domain_discriminator_accuracy
            cdan_losses[iter_]= Align_dict['cdan'].domain_discriminator_accuracy

            mean_losses[iter_] = np.mean(losses[max(0, iter_ - 100) : iter_ + 1])
            mean_dann_losses[iter_] = np.mean(dann_losses[max(0, iter_ - 100) : iter_ + 1])
            mean_cdan_losses[iter_] = np.mean(cdan_losses[max(0, iter_ - 100) : iter_ + 1])
            ...
            if display_iter and iter_ % display_iter == 0:
                ...
                loss_dann = display.line(
                    X=np.arange(iter_ - display_iter, iter_),
                    Y=mean_dann_losses[iter_ - display_iter : iter_],
                    win=loss_dann,
                    update=update,
                    opts={
                        "title": "Training dann loss",
                        "xlabel": "Iterations",
                        "ylabel": "Loss",
                    },
                )
                loss_cdan = display.line(
                    X=np.arange(iter_ - display_iter, iter_),
                    Y=mean_cdan_losses[iter_ - display_iter : iter_],
                    win=loss_cdan,
                    update=update,
                    opts={
                        "title": "Training cdan loss",
                        "xlabel": "Iterations",
                        "ylabel": "Loss",
                    },
                )
    ...
    # torch.load(net.state_dict(), model_dir + "best" + ".pth")     
...
def val(net, data_loader, device="cpu", supervision="full"):
    ...
    for batch_idx, (data, target,ord,weight) in enumerate(data_loader):
        with torch.no_grad():
            # Load the data into the GPU if required
