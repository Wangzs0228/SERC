...
from models0313 import get_model, train, test, save_model,Mytest,val
...
group_train.add_argument("--visname",type=str,default="raw",)
group_train.add_argument("--sample_ratio",type=float,default=1,)
group_train.add_argument("--class_match",type=int,default=1,)
...
viz = visdom.Visdom(env=DATASET + " " + MODEL + args.visname)
...
train_gt, _,_,_ = sample_gt(gt, hyperparams["sample_ratio"], mode="random")
train_t_gt, _,_,_  = sample_gt(gt_t, 1, mode='random')
val_gt, _,_,_  = sample_gt(gt_t, 1, mode='random')
...
# display_dataset(img_t, gt_t, RGB_BANDS_T, LABEL_VALUES, palette, viz)
...
for run in range(N_RUNS):
    ...
    acc_s, f_s, y_s, label_s = val(model, train_loader,hyperparams["device"],hyperparams["supervision"])
    acc_t, f_t, y_t, label_t = val(model, val_loader2, hyperparams["device"],hyperparams["supervision"])
    wandb.log({"acc_s":acc_s,"acc_t":acc_t})
    ...
    if DATASET!="Hangzhou":
        ...
        visualize(f_s.cpu().detach()[0:num], f_t.cpu().detach()[0:num],y=torch.cat((label_s[0:num],label_t[0:num]),0),label=list(range(1, N_CLASSES + 1)),name=DATASET)

    flops_FE, params_FE = profile(model, (torch.randn(1, train_dataset[0][0].shape[0], train_dataset[0][0].shape[1], train_dataset[0][0].shape[2],train_dataset[0][0].shape[3]).cuda(),))
    print('flops: %.2f M, params: %.2f M' % ((flops_FE) / 1e6, (params_FE) / 1e6))
    print('-' * 100, '\nTesting')
    ...
    wandb.log({"bestval":bestval,"mybestval":mybestval})
if N_RUNS > 1:
