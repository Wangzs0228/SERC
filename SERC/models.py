...
def train(
    ...
    for e in tqdm(range(1, epoch + 1), desc="Training epoch"):
        ...
        for batch_idx in tqdm(range(0, total), desc="Training the network"):
            ...
            x_t, labels_t,ord_t,weight_t = data_t.to(device), target_t.to(device),ord_t.to(device),weight_t.to(device)
            ...
            if supervision == "full":
                ...
                loss = (criterion(output_s, labels_s)*weight_s).mean()\
                +1*Align_dict['mcc'](output_t)
