...
def train(
    ...
    for e in tqdm(range(1, epoch + 1), desc="Training epoch"):
        ...
        for batch_idx in tqdm(range(0, total), desc="Training the network"):            
            ...
            eff = calc_coeff(max(max(e,0)*total+batch_idx-len(val_loader)/len(data),0), max_iter=total*epoch)
            ...
            if supervision == "full":
                ...
                output_s, output_t,output_t_raw = output.chunk(3, dim=0)
                ...
                logits_ulb_w, logits_ulb_s = output_t_raw,output_t
                ...
                loss +=hyperparams["sat"]*loss_sat + hyperparams["saf"]*loss_saf
                # net.eval() 
                with torch.no_grad():
            ...
            acc_raw +=((torch.max(output_t,1)[1]==labels_t)+0).sum()
            acc_nc  +=((pred==labels_t)+0).sum()
            if display_iter and iter_ % display_iter == 0:
                string = "Train (epoch {}/{}) [{}/{} ({:.0f}%)]\tLoss: {:.6f}\taccraw:{}\taccna:{}\teff:{:.2f}"
            ...
            'CDAN_acc': Align_dict['cdan'].domain_discriminator_accuracy,})
                ...
                if len(val_accuracies) > 0:
                    val_win = display.line(
                        Y=np.column_stack((np.array(val_accuracies),np.array(myval_accuracies))),
                        X=np.column_stack((np.arange(len(val_accuracies)),np.arange(len(val_accuracies)))),
                        win=val_win,
                        opts={
                ...
                # if len(myval_accuracies) > 0:
                #     val_win = display.line(
                #         Y=np.array(myval_accuracies),
                #         X=np.arange(len(myval_accuracies)),
                #         win=val_win,
                #         name="na",
        ...
        if val_loader is not None:
            ...
            wandb.log({'val_accs':val_acc,'myval_accs':myval_acc,"acc_raw":acc_raw
            ,"acc_nc":acc_raw,"eff":eff })
            metric = -val_acc
        else:
            metric = avg_loss
