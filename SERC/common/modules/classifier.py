...
class Classifier(nn.Module):
    ...
    def classifier(self, features: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """"""
        features = self.bottleneck(self.pool_layer(features))
        outputs = self.head(features)
        return outputs
    
    def domain_feature(self, features: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """"""
        features = self.bottleneck(self.pool_layer(features))
        return features
