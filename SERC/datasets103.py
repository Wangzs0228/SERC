...
def get_dataset(dataset_name, target_folder="./", datasets=DATASETS_CONFIG):
    ...
    if dataset_name == 'Houston13':
        # Load the image
        img = open_file(folder + 'Houston/Houston13.mat')['ori_data']

        rgb_bands = (43, 21, 11)

        gt = open_file(folder + 'Houston/Houston13_7gt.mat')['map']
        ...
        # Load the image
        img = open_file(folder + 'Houston/Houston18.mat')['ori_data']

        rgb_bands = (43, 21, 11)

        gt = open_file(folder + 'Houston/Houston18_7gt.mat')['map']
        ...
        # Load the image
        img = open_file(folder + 'ShangHai/Shanghai.mat')['DataCube1']

        rgb_bands = (43, 21, 11)

        gt = open_file(folder + 'ShangHai/Shanghai_gt.mat')['gt1']
        ...
        # Load the image
        img = open_file(folder + 'ShangHai/Hangzhou.mat')['DataCube2']

        rgb_bands = (43, 21, 11)

        gt = open_file(folder + 'ShangHai/Hangzhou_gt.mat')['gt2']
        ...
        # Load the image
        img = open_file(folder + 'hyrank/Dioni.mat')['ori_data']

        rgb_bands = (43, 21, 11)

        gt = open_file(folder + 'hyrank/Dioni_gt_out68.mat')['map']
        ...
        # Load the image
        img = open_file(folder + 'hyrank/Loukia.mat')['ori_data']

        rgb_bands = (43, 21, 11)

        gt = open_file(folder + 'hyrank/Loukia_gt_out68.mat')['map']
        ...
        # Load the image
        img = open_file(folder + 'pavia/paviaU.mat')['ori_data']

        rgb_bands = [20, 30, 30]

        gt = open_file(folder + 'pavia/paviaU_7gt.mat')['map']  # 加载原版标签
        ...
        # Load the image
        img = open_file(folder + 'pavia/paviaC.mat')['ori_data']

        rgb_bands = [20, 30, 30]

        gt = open_file(folder + 'pavia/paviaC_7gt.mat')['map']
    ...
    return img, gt, label_values, ignored_labels, rgb_bands, palette        #= CUSTOM_DATASETS_CONFIG[dataset_name]['loader'](folder)
...
#         # Load the image
#         img = open_file(folder + "Pavia.mat")["HSI_original"]

#         rgb_bands = (55, 41, 12)

#         gt = open_file(folder + "Pavia_gt.mat")["Data_gt"]
...
class HyperX(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        data_raw = np.asarray(np.copy(data_raw).transpose((2, 0, 1)), dtype='float32')
        ...
        if self.center_pixel and self.patch_size > 1:
            labels = label[self.patch_size // 2, self.patch_size // 2]
            weight = len(label[label==labels])/self.patch_size/self.patch_size
            label  = labels
        ...
        return data_raw.unsqueeze(0), data, label,ord, weight, i
